#!/usr/bin/env python3
"""
Production Installation & Update Script for battld Game Server
...
PROJECT_ROOT = Path("/root/battld")
SERVER_BINARY_SOURCE = PROJECT_ROOT / "target" / "release" / "server"
SERVER_BINARY_DEST = Path("/usr/local/bin/battld-server")
...
STATIC_SOURCE = PROJECT_ROOT / "server" / "static"
STATIC_DEST = DATA_DIR / "server" / "static"
DOMAIN = "battld.curzel.it"
...
def create_env_file():
    ...
    env_content = f"""# battld Server Production Environment Configuration
# Database location
DATABASE_URL=sqlite://{DATA_DIR}/game.db
...
Description=battld Game Server
After=network.target
Documentation=https://github.com/yourusername/battld
...
def run_migrations():
    ...
    db_url = f"sqlite://{DATA_DIR}/game.db"
...
def obtain_ssl_certificate():
    ...
    print("  3. The battld service will be temporarily stopped")

    # Stop the service to free up port 80
    print("\nStopping battld service temporarily...")
...
def setup_certbot_renewal():
    ...
    hook_script = hooks_dir / "restart-battld.sh"
    hook_content = f"""#!/bin/bash
# Restart battld server after certificate renewal
...
def print_summary():
    ...
    print("\n" + "="*60)
    print("🎮 battld Server Installation/Update Complete!")
    print("="*60)
    print(f"\nService Management:")
...
def main():
    ...
    print("="*60)
    print("battld Game Server - Production Installation")
    print("="*60)
